...
{#pointer *CTrainer as Trainer
    foreign finalizer delete_SimpleSGDTrainer newtype #} -- TODO
...
{#fun init_SimpleSGDTrainer as createSimpleSGDTrainer
    {+S, `Model', `Float', `Float'} -> `Trainer' #}

{#fun Trainer_update as update {`Trainer', `Float'} -> `()' #}
...
instance Storable Trainer where
...
{#fun new_Dim_v as ^ {withVector `Vector Int64'} -> `HDim' #}
